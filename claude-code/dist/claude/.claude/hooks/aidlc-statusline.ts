...
import { loadAgents } from "../tools/aidlc-lib.ts";
...
async function resolveProjectDir(input: Input): Promise<string> {
  ...
  // Method 2: CLAUDE_PROJECT_DIR env var
  if (process.env.CLAUDE_PROJECT_DIR) return process.env.CLAUDE_PROJECT_DIR;

  // Method 3: derive from script path (strip .claude/hooks)
  const scriptDir = dirname(fileURLToPath(import.meta.url));
  const suffix = `${sep}.claude${sep}hooks`;
  if (scriptDir.endsWith(suffix)) {
    return scriptDir.slice(0, -suffix.length);
  }

  // Method 4: CWD has .claude directory
...
function abbreviateModel(modelId: string): string {
  ...
  let short = modelId;
  let prefix = "";
  if (short.startsWith("us.anthropic.")) {
    prefix = "BR:";
    short = short.slice("us.anthropic.".length);
  }
