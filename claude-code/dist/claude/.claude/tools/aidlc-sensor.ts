...
// `command:` is `bun .claude/tools/aidlc-sensor-<id>.ts` — the dispatcher
...
//
// Manifest `command:` is `bun .claude/tools/aidlc-sensor-<id>.ts`. The
...
function matchesGlob(pattern: string, path: string): boolean {
	...
	return variants.some((v) => globToRegex(v).test(path));
}
