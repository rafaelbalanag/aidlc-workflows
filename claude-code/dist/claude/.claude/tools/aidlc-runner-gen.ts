...
function runnableStages(): GraphStage[] {
...
function renderStageRunner(node: GraphStage): string {
...
1. Ask the engine for the single-stage directive:
   ...
   \`\`\`bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage ${node.slug} --single
   \`\`\`
...
2. When the stage's work is done, commit the single-stage record:
   ...
   \`\`\`bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage ${node.slug} --result completed
   \`\`\`
...
function renderInitRunner(): string {
  return `---
...
function handleCheck(): void {
  ...
  console.log("Run `bun .claude/tools/aidlc-runner-gen.ts write` to regenerate.");
  process.exit(1);
}
...
1. \`directive = bun .claude/tools/aidlc-orchestrate.ts next --scope ${scope} $ARGUMENTS\`
