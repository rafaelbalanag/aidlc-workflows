...
function learningsFilePath(projectDir: string, scope: "project" | "team"): string {
  return join(projectDir, ".claude", "rules", `aidlc-${scope}-learnings.md`);
}
...
function sensorManifestPath(projectDir: string, sensorId: string): string {
  return join(projectDir, ".claude", "sensors", `aidlc-${sensorId}.md`);
}
...
function stagesDir(projectDir: string): string {
  return process.env.AIDLC_STAGES_DIR ?? join(projectDir, ".claude", "aidlc-common", "stages");
}
...
// state's Test-Run-Mode signal. Stored as `- **Test-Run Mode**: true` in
// the state file (mirrors aidlc-state.ts's gate Test-Run field semantics).
...
function isFrameworkDistributionPath(path: string): boolean {
  return path.includes(join("dist", "claude", ".claude", "sensors"));
}
