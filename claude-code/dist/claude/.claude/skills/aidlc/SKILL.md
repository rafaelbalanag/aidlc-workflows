...
| `ask` | Render `directive.question` via `AskUserQuestion`, then feed the human's answer back on the next `report` via `--user-input "<answer>"`. The engine never calls `AskUserQuestion` itself — it defers the human turn to you. |
...
- **`gate: true`** — after the stage body produces its artifacts, run the **§13 learnings ritual** then present the **approval gate**:
  ...
  3. Present the approval gate via `AskUserQuestion` (Approve / Request Changes). On approval, `report --result approved` — the engine's `report` owns the full transition (it dispatches the right `aidlc-state.ts` subcommand and advances). On a Request-Changes / reject, run the Keep/Modify/Redo loop within this stage (below) and re-present; the reject path stays conductor-side and is not a `report` outcome.
