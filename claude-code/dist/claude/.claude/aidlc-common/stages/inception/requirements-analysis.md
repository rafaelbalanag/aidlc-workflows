...
2. Confirm ALL `[Answer]:` tags are filled in. If any are blank, present the unanswered questions via AskUserQuestion and write answers back. Do NOT proceed with partial answers.
...
AskUserQuestion({
  questions: [{
    ...
    options: [
      ...
      { label: "Add User Stories", description: "Include User Stories stage (currently skipped)" }
    ]
  }]
})
```
