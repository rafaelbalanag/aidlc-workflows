...
condition: Always rerun for freshness. Brownfield discovers from evidence + reverse-engineering artifacts. Greenfield prompts user via AskUserQuestion using org.md defaults.
...
outputs: "aidlc-docs/inception/practices-discovery/ (4 artifacts: team-practices.md, discovered-rules.md, evidence.md, practices-discovery-timestamp.md). On affirmation, content is promoted to .claude/rules/aidlc-team.md and .claude/rules/aidlc-project.md."
---
...
Use AskUserQuestion to surface five practice areas, one per `aidlc-team.md` section heading: Way of Working, Walking Skeleton, Testing Posture, Deployment, Code Style.
...
**Test-run mode**: per `stage-protocol.md`, skip AskUserQuestion calls. For greenfield, use `aidlc-org.md` defaults verbatim. For brownfield, use evidence-only findings. Log via `bun dist/claude/.claude/tools/aidlc-log.ts answer --test-run` for each implied answer.

Log each question via `bun dist/claude/.claude/tools/aidlc-log.ts decision` BEFORE issuing AskUserQuestion. Log each answer via `bun dist/claude/.claude/tools/aidlc-log.ts answer` after the user responds.
...
After writing the four artifacts, emit `PRACTICES_DISCOVERED` via `bun dist/claude/.claude/tools/aidlc-state.ts practices-event --type discovered --field "Sources Scanned: <list>" --field "Drafts: team-practices.md, discovered-rules.md"` (the tool wraps the audit emission so events stay tool-owned per the audit-first invariant).

### Step 5: Affirmation Gate

Compliance with `stage-protocol.md` checklist:

1. `bun dist/claude/.claude/tools/aidlc-state.ts gate-start practices-discovery` BEFORE the AskUserQuestion.
2. `bun dist/claude/.claude/tools/aidlc-log.ts decision` for the affirmation question.
3. AskUserQuestion presents `team-practices.md` and `discovered-rules.md` for review. Options:
...
4. `bun dist/claude/.claude/tools/aidlc-log.ts answer` after the user answers.
5. `bun dist/claude/.claude/tools/aidlc-state.ts approve --user-input "<exact label>"` (or `reject`) — auto-emits the gate-approved/gate-rejected audit events.
6. **Test-run mode**: `bun dist/claude/.claude/tools/aidlc-state.ts approve --user-input "Approve (test-run)" --test-run`.
...
```
bun dist/claude/.claude/tools/aidlc-state.ts practices-promote \
...
2. Update `Practices Affirmed Timestamp` in `aidlc-docs/aidlc-state.md` via `bun dist/claude/.claude/tools/aidlc-state.ts set "Practices Affirmed Timestamp=NOW"` (the `NOW` literal expands to the current ISO 8601 timestamp; the field is part of the v7 state template).
...
- AskUserQuestion with options: Approve (continue to Requirements Analysis) / Request Changes
