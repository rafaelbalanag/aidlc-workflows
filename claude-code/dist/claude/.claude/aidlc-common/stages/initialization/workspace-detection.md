...
The scanner walks the project directory one level deep plus known source directories (`src/`, `app/`, `lib/`, `pages/`, `components/`, `tests/`), excluding `.claude/`, `aidlc-docs/`, `node_modules/`, `.git/`, `dist/`, `build/`, `.next/`, `target/`, `vendor/`.
...
- `.claude/` — AI-DLC framework directory (skills, agents, hooks, tools, knowledge)
...
Does NOT make a project brownfield: README, .gitignore, LICENSE, editor configs, empty directories, CI/CD boilerplate without application code, `.claude/` directory (AI-DLC framework), `aidlc-docs/` directory (AI-DLC artifacts).
