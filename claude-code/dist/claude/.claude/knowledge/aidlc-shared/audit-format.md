...
| `STAGE_AWAITING_APPROVAL` | Stage enters `[?]` (gate open) | Timestamp, Stage, Artifacts | `tools/aidlc-state.ts gate-start` |
...
| Event | When | Required Fields | Emitter |
|-------|------|-----------------|---------|
| `DECISION_RECORDED` | Before AskUserQuestion, to record the options shown | Timestamp, Stage, Decision, Options, optional `Test-Run=true` | `tools/aidlc-log.ts decision` |
