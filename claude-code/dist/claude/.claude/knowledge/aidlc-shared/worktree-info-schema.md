...
Pinned schema and exit-code contract for the `info` subcommand. The orchestrator's halt-and-ask prose at `SKILL.md` reads this output to interpolate the worktree path and branch name into the AskUserQuestion prompt body for code-generation-failure halt-and-ask.
...
The orchestrator interpolates `path` and `branch_name` into the AskUserQuestion `question` body, which renders at full terminal width and wraps gracefully (multi-line wrap is supported on macOS Claude Code; verified manually before each release).
