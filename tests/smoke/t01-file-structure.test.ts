...
//   .sh L53-55  7 construction stages (loop)           -> "ships the 6 construction stages"
...
describe("t01 — shipped-tree file-structure invariant (mechanism: none)", () => {
  ...
  test("ships the 6 construction stages [.sh L53-55]", () => {
  ...
  // (32 = the original 32 + contract-design - nfr-requirements; the nfr merge
  // in RFC-0001 folds nfr-requirements into nfr-design.)
  ...
  test("asserts EXACTLY 65 shipped paths (TAP plan 63 + 2 reviewer agents + contract-design - nfr-requirements) [.sh L9]", () => {
    const paths: string[] = [
