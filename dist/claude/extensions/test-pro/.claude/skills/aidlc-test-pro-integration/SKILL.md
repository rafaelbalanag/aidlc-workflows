---
name: aidlc-test-pro-integration
description: >
  Run the AI-DLC `test-pro-integration` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage test-pro-integration --single`:
  the engine emits one run-stage directive for test-pro-integration and its gate, the
...
# AI-DLC Stage Runner — test-pro-integration

Run the `test-pro-integration` stage on its own. This is opt-in packaging over
`/aidlc --stage test-pro-integration --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage test-pro-integration --single
   ```

   The engine emits one `run-stage` directive for `test-pro-integration` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage test-pro-integration --result completed
   ```
