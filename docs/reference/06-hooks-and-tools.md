...
| `intent-birth` | Birth an intent and run the Initialization phase (workspace scaffold, workspace detection, state init). Accepts `--scope <scope>`, `--depth`, and `--test-strategy`. | `WORKFLOW_STARTED`, `PHASE_STARTED`, `PHASE_SKIPPED`, `STAGE_STARTED`, `STAGE_COMPLETED`, `WORKSPACE_*`, and the init-to-first-post-init phase hand-off events |
